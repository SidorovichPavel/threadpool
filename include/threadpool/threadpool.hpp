...
class threadpool final {
 public:
  ...
  std::future<std::invoke_result_t<Fn, Args...>> enqueue(Fn&& fn,
                                                         Args&&... args);
  ...
  std::future<invoke_result_t> res = pTask->get_future();
  {
    ...
    if (stop_pool_)
      throw std::runtime_error(
          "ThreadPool, push task failed. How did you do it?");

    tasks_.emplace([utask = std::move(pTask)] { (*utask)(); });
  }
